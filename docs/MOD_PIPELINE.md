...
| **Face tears or spikes when he blinks**                             | Morph deltas address vertices by _local index_, and the exporter renumbers them. They are now remapped through the `po_vertex_source` attribute the importer stamps on every vertex, so each delta goes back to the archive vertex it was authored for, even if you moved it. Vertices without it (older imports, geometry you added) fall back to matching by position.
| **Hair shards float off, puffy eyes or a wrong idle mouth**        | Coincident vertices: a hair flap lying on the scalp, an eyelid over its socket. Matched by position, one vertex's morph delta was copied onto every vertex at that point, so the surface underneath moved with it when the shape fired. Fixed by the identity remap above. **Re-import scenes made before this fix**, or the exporter still has only positions to go on. When you reshape, also keep head, face and hair vertices out of any smoothing or proportional edit on the body: moving them changes the base the face shapes are added to.
